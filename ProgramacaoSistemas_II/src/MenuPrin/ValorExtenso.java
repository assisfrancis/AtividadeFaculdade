...
public class ValorExtenso {
    ...
    public static String get(BigDecimal numero, String qualificadorSingular,
        String qualificadorPlural, String qualificadorDecimalSingular,
        String qualificadorDecimalPlural) {
        String extenso = "";
        String conector = "";
        long parteInteira = numero.longValue();
        if (parteInteira > 0) {
         extenso = get(parteInteira, qualificadorSingular, 
         qualificadorPlural);
          conector = " e ";
        }
    long parteDecimal = numero.remainder(BigDecimal.ONE).movePointRight(
                numero.scale()).longValue();
        if (parteDecimal > 0) {
            extenso += conector + get(parteDecimal, 
                    qualificadorDecimalSingular, 
                    qualificadorDecimalPlural);
        }
        return extenso;
    }
    public static String get(long numero, String qualificadorSingular, 
        String qualificadorPlural) {
        String qualificador = numero == 1 ? qualificadorSingular : 
        qualificadorPlural;
    ...
    private static List<Classe> buildClassesFromAlgarismos(int[] 
    rawAlgarismos) {
        ...
        for (int algarismoPos = rawAlgarismos.length - 1; algarismoPos >= 0; 
                algarismoPos--) {
        Algarismo algarismo = AlgarismoFactory.create(casa, rawAlgarismos[
              algarismoPos], algarismoADireita, classe);
    ...
    private static String buildExtensoFromClasses(List<Classe> classes) {
        ...
        for (Classe classeZ : classes) {
            ...
            for (Algarismo algarismo : classeZ.algarismos) {
                extensoClasse.insert(0, algarismo.conector() + 
                        algarismo.representacao());
            }
            extenso.insert(0, classeZ.conector() + extensoClasse + 
                    classeZ.representacao());
        }
...
static Algarismo create(int casa, int valor, Algarismo algarismoADireita,
            Classe classe) {
        try {
           return (Algarismo) casas.get(casa)
              .getConstructor(int.class, Algarismo.class,
                      Classe.class)
...
private static final String[] unidadePorExtenso = {"", "um", "dois", "três",
...
class Centena extends Algarismo {
    ...
    String representacao() {
        if (valor == 1
                && ((algarismoADireita.valor > 0) || (
                algarismoADireita.algarismoADireita.valor > 0))) {
            return CENTO;
        }
...
//*************************************************************
class Classe {
    private static final String[] classeSingular = {"", " mil", " milhão", " bilhão",
        " trilhão", " quatrilhão"};
    private static final String[] classePlural = {"", " mil", " milhões", " bilhões",
